...
# Функция для добавления записи с датой текущего дня в лист для текущего месяца
def add_date_record_to_monthly_sheet(monthly_sheet, current_datetime):
    date = current_datetime.strftime("%Y-%m-%d")
    data = [date, "", "", ""]  # Пустые поля для пользователя, типа и комментария
...
def process_comment_step(message):
    try:
        ...
        if monthly_sheet.row_count == 1:
            add_date_record_to_monthly_sheet(monthly_sheet, current_datetime)
        ...
        admin_notification = f"Пользователь {user_info} записал информацию в гугл-таблицу.\n{notification_message}"
        bot.send_message(admin_username, admin_notification)
